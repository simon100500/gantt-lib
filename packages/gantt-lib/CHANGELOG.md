...
- New `onSubtreeScaleResult` prop (gantt mode): structured scaling result — clamp flag, `TARGET_CLAMPED_TO_MINIMUM` warning, error code — for consumer-side localized notifications
- The "preserve duration" checkbox (`taskDateChangeMode`) now governs parent date edits in the TaskList: `preserve-duration` moves the stage (uniform descendant shift in project days), `free` changes the duration (proportional subtree scaling); boundary deltas are measured in project days, fixing weekend-crossing shifts in business mode
...
- Critical-path "hide" filter now also filters the task list (TaskList), keeping its height and the add-task button in sync
...
- Add task multi-select support with `enableTaskMultiSelect`, `selectedTaskIds`, and `onSelectedTaskIdsChange`
