...
- Transient-координаты drag/cascade теперь очищаются при замене controlled-расписания, поэтому стрелки зависимостей остаются привязаны к authoritative-позициям задач
- Ручки создания связей отключены в touch/coarse-pointer раскладках, чтобы мобильный скролл не перехватывался элементами связей
...
- Исправлена production-сборка сайта: помощник вывода дат демо теперь хранит длительность в рабочих днях на производных листовых задачах (тип `Task & { durationDays }`), поэтому проверка типов больше не падает при обращении к `leaf.durationDays`

## [0.128.1] - 2026-08-24

### Исправления

- Перетаскивание родительской задачи: потомки сдвигаются жёстко вместе с родителем, сохраняя внутренние зазоры и лаги — раньше повторное применение ограничений зависимостей к уже сдвинутым потомкам схлопывало зазоры и приводило к дрейфу потомков за границу родителя
- Переключение режима рабочих дней (`reflowTasksOnModeSwitch`): длительность задачи теперь измеряется в целевом режиме, поэтому включение рабочих дней больше не раздувает задачи на выходные внутри них и не сдвигает позднего FS-преемника раньше предшественника (что инвертировало стрелку связи); демо-данные выводятся из длительности, связи и лага, поэтому хранимые лаги всегда совпадают с нарисованным зазором
...
- Чекбокс «сохранять длительность» (`taskDateChangeMode`) управляет редактированием дат родителя в TaskList: `preserve-duration` — двигаем срок (равномерный сдвиг потомков в проектных днях), `free` — меняем длительность (пропорциональное масштабирование поддерева); дельты границ считаются в проектных днях, что чинит сдвиг через выходные в business-режиме
...
- Упрощение управления состоянием окна дат в режиме план-факт

## [0.121.1] - 2026-06-05

### Исправления

- Стабилизация гидратации для vh-контейнеров

### Производительность

- Виртуализация окна дат в режиме план-факт
...
- Исправлено позиционирование контекстного меню и поведение подменю в TaskList
...
- Группировка ресурсов по типу — проп `resourceGrouping` (`false | 'type'`) с разделительными строками групп в tasklist и сетке таймлайна
- Циклический перебор назначений ресурса с улучшенным скроллингом

### Исправления

- Исправлено отображение отработанных дней и количества назначений в resource planner

### Стили

- Добавлен padding для метрики ресурса, улучшено выравнивание
- Убрана граница TaskList, добавлен overlay для лучшего визуального разделения
