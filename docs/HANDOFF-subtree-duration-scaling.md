...
> Контракт релиза **v0.128.0**. Как интегрировать функцию «изменение родительской
> задачи по длительности» и как коммитить полученные изменения дат в хранилище
> через `onTasksChange`.
...
## 4. Как коммитить изменения дат (`onTasksChange`)
...
## 5. Сценарий B — встроенный UI (`<GanttChart>`)
